 ...
 * 
 * For: Real Estate Investor Flipper Platform
 */
...
const MANUS_OUTPUT_PATH = join(process.cwd(), "docs", "discovery", `MANUS-${PROJECT_SLUG}.md`);
...
async function orchestrateManusDiscovery() {
  ...
  // Step 1: Verify Raw Manus Output Exists
  console.log("📥 Step 1: Verifying Raw Manus Discovery Pack...\n");
  
  if (!existsSync(MANUS_OUTPUT_PATH)) {
    console.error(`❌ Manus discovery pack not found at: ${MANUS_OUTPUT_PATH}`);
    console.error("   Please run Step 1 first: npx tsx scripts/start-real-estate-discovery.ts");
    process.exit(1);
  }

  const rawManusContent = readFileSync(MANUS_OUTPUT_PATH, "utf-8");
  console.log(`✅ Found raw Manus output: ${MANUS_OUTPUT_PATH}\n`);
...
**Manus Raw Output:**
${rawManusContent.substring(0, 10000)}${rawManusContent.length > 10000 ? "..." : ""}

**Available Citations from Manus:**
${sourcesList}

${chatgptRefinementContent ? `**ChatGPT Refinement:**
${chatgptRefinementContent.substring(0, 5000)}${chatgptRefinementContent.length > 5000 ? "..." : ""}` : "**Note**: ChatGPT refinement was skipped. Critique based on raw Manus output."}
...
**Target Niche**: ${NICHE}  
**Generated**: ${new Date().toISOString().split('T')[0]}  
**Source**: Claude Red-Team Critique  
...
`;
  ...
  if (missingDocuments.length > 0) {
    ...
    console.log();
    console.log("🚨 ORCHESTRATION INCOMPLETE: Step 4 (Cursor Agents) not completed!\n");
    console.log("📋 Required Actions:");
    console.log("   According to Rule 311 (Manus Orchestration Flow), you MUST complete Step 4:");
    ...
    console.log(`      Inputs: ${MANUS_OUTPUT_PATH} + ${chatgptOutputPath || "ChatGPT refinement skipped"}`);
    console.log();
    ...
    console.log(`   - Raw Manus output: ${MANUS_OUTPUT_PATH}`);
  ...
  console.log(`   1. ${MANUS_OUTPUT_PATH}`);
  if (chatgptOutputPath) {
