...
export default async function ResearchDocumentPage({ params }: PageProps) {
  const { path: pathArray } = await params;
  const path = pathArray.join("/");
  
  let content: string | null = null;
  
  // Try research document path
  const possiblePaths = [
    join(process.cwd(),"docs","research", path +".md"),
    join(process.cwd(),"docs","research", path),
  ...
  const title = filename
    .replace(".md","")
    .replace(/^(PERSONA|COMPETITORS|MARKET|RESEARCH|SYNTHESIS)-/i,"")
    .replace(/-/g,"")
        ...
        <div className="glass-card rounded-lg p-8 lg:p-12">
          ...
          <div className="mb-8">
            <div className="flex items-center gap-3 mb-4">
              ...
              <span className="px-2 py-1 text-xs font-semibold rounded bg-orange-500/20 text-orange-400 border border-orange-500/50">
                RESEARCH
              </span>
            </div>
            <div className="flex items-start gap-4">
              <div className="w-12 h-12 rounded-lg bg-orange-500/20 border border-orange-500/50 flex items-center justify-center flex-shrink-0">
          ...
          {/* Related Documents */}
          <RelatedDocuments currentPath={`research/${path}`} projectSlug={projectSlug || undefined} />
        </div>
      </div>
    </div>
  );
