        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          {relatedDocs.map((doc) => (
            <Link
              key={doc.path}
              href={`/docs/${doc.path}`}
              className="group flex items-center justify-between p-3 rounded-lg bg-slate-800/30 hover:bg-slate-800/50 border border-slate-700 hover:border-slate-600 transition-all duration-300"
            >
              ...
              <ArrowRight className="w-4 h-4 text-gray-500 group-hover:text-blue-400 transition-colors flex-shrink-0 ml-2" />
            </Link>
          ))}
        </div>
      </CardContent>
